...
1. Опишите самую интересную задачу в программировании, которую вам приходилось решать?
    ...
    Разрабатывал телеграм-бота, которому необходимо было запоминать состояние чата для каждого пользователя, чтобы корректно отвечать, учитывая определенные входные данные и текущее состояние.
2. Расскажите о своем самом большом факапе? Что вы предприняли для решения проблемы?

    В первых версиях своих веб-приложениях хранил и пушил в git секретные данные, такие как различные токены и пароли. Сейчас тщательно слежу, чтобы все секреты хранились в переменных окружения.
3. Каковы ваши ожидания от участия в буткемпе?

    Я ожидаю обучиться у профессионалов тонкостям микросервисной архитектуры, поработать над проектом с grpc, kafka, k8s. Кроме того хочу проявить себя на буткемпе и получить оффер в компанию.
---

## Чтобы поднять сервис необходимо ввести 2 команды:
>docker-compose build

>docker-compose up

По пути *cmd/simple-client/main.go* находится простой grpc-client, изменяя и используя который можно отправлять запросы сервису.
